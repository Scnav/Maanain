        ...
        if (mensagens.length === 0) {
            ...
            container.innerHTML = `
                <table class="users-table">
                    <thead>
                        <tr><th>Título</th><th>Conteúdo</th><th>Vídeo</th><th>Data</th><th>Status</th><th>Ações</th></tr>
                    </thead>
                    <tbody>
                        ${mensagens.map(msg => `
                            <tr>
                                <td><strong>${msg.titulo}</strong></td>
                                <td>${msg.conteudo ? msg.conteudo.substring(0, 50) + '...' : '-'}</td>
...
function editarMensagem(id, titulo, conteudo, video_url, ativa) {
    ...
    document.getElementById('mensagemConteudo').value = conteudo || '';
...
async function salvarMensagem() {
    ...
    try {
        ...
        const response = await fetch(url, {
            method: method,
            headers: getAdminHeaders(),
            body: JSON.stringify({ titulo, conteudo, video_url, ativa })
        });
